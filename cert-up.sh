...
checkCertExpiry() {
  echo 'begin checkCertExpiry'
  # 获取证书的到期日期
  EXPIRY_DATE=$(openssl x509 -enddate -noout -in ${CRT_PATH}/cert.pem | cut -d= -f2)
  # 将到期日期转换为时间戳
  EXPIRY_TIMESTAMP=$(date -d "${EXPIRY_DATE}" +%s)
  # 获取当前日期的时间戳
  CURRENT_TIMESTAMP=$(date +%s)
  # 计算到期日期与当前日期的差值（以天为单位）
  DAYS_LEFT=$(( (EXPIRY_TIMESTAMP - CURRENT_TIMESTAMP) / 86400 ))

  echo "Certificate expires in: ${DAYS_LEFT} days"
  if [ ${DAYS_LEFT} -eq 0 ]; then
    echo "Certificate expires today. Proceeding with update..."
    updateCrt
  else
    echo "Certificate does not expire today. No action taken."
  fi
}
...
case "$1" in
  ...
  check-expiry)
    ...
    *)
        echo "Usage: $0 {update|revert|check-expiry}"
        exit 1
esac
